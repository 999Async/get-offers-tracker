...
--> statement-breakpoint
CREATE INDEX `job_versions_job_idx` ON `job_versions` (`job_id`);--> statement-breakpoint
...
--> statement-breakpoint
CREATE TRIGGER job_versions_immutable_update BEFORE UPDATE ON job_versions
BEGIN SELECT RAISE(ABORT, 'immutable job version'); END;
--> statement-breakpoint
CREATE TRIGGER job_versions_immutable_delete BEFORE DELETE ON job_versions
BEGIN SELECT RAISE(ABORT, 'immutable job version'); END;
--> statement-breakpoint
CREATE TRIGGER job_corpus_immutable_update BEFORE UPDATE ON job_corpus_snapshots
